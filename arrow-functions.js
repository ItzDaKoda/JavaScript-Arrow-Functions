...
const processOrderArrow = (item, quantity) => {
    ...
    // Note: the starter function above doesn't include "$", but your test expects it.
    // If your instructor wants it to match the starter function exactly, remove the "$" below.
    return `Order total: $${finalAmount.toFixed(2)}`;
};
...
const calculateSalePrice = (name, price) => {
    const salePrice = price * 0.8; // 20% discount
    // Test expects "$80" not "$80.00", so avoid forcing 2 decimals.
    const formatted = Number.isInteger(salePrice) ? salePrice.toString() : salePrice.toFixed(2);
    return `Product: ${name} - Sale Price: $${formatted}`;
};
